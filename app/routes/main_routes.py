...
def register_session():
	if request.method == 'POST':
		...
		precio_hora = float(data.get('precioHora', 0))
		total = float(data.get('total', 0))
		
		# Registrar sesión con los nuevos campos
		db.Sessions.insert_one({
			'id': provider_id, 
			'Date': fecha, 
			'NumberSessions': cantidad,
			'PricePerHour': precio_hora,
			'TotalAmount': total
			})
		# Comprobar si hay modificacion de precio x hora

		chk_ph = True if db.Providers.find_one({'id': provider_id})['SessionPrice'] != float(precio_hora) else None
		if chk_ph:
			db.Providers.update_one({'id': provider_id}, {'$set': {'SessionPrice': precio_hora}})
...
def make_payment():
	...
	if selected_provider:
		...
		if provider:
			...
			amount_due = list(db.Sessions.aggregate([{'$match': {'id': selected_provider,'Payment': {'$exists': False} }},
				{'$group': {'_id': None, 'amnt': {'$sum': '$TotalAmount'}}}]))[0]['amnt'] if len(sessions) > 0 else 0
